            ...
            for link in links:
                ...
                try:
                    ...
                    for channel in channels:
                        await app.send_document(chat_id=channel, document=filename, caption=f"Uploaded: {filename}")
...
async def set_channels_cmd(client, message):
    if message.from_user.id != OWNER_ID:
        await message.reply("❌ You are not authorized to change channels.")
        return

    if len(message.command) < 2:
        await message.reply("⚠️ Usage: /channels @channel1-@channel2-@channel3")
        return

    raw_text = message.text.split(" ", 1)[1]
    channel_list = [ch.strip() for ch in raw_text.split("-") if ch.strip()]
    set_channels(channel_list)
    await message.reply(f"✅ Updated channels list:\n\n" + "\n".join(channel_list))
...
async def set_urls_cmd(client, message):
    ...
    raw_text = message.text.split(" ", 1)[1]
    url_list = [url.strip() for url in raw_text.split("-") if url.strip()]
